...
class StudentProfileAPITests(APITestCase):
    def setUp(self):
        ...
        self.list_create_url = reverse('student-profile-list')
        self.detail_url = reverse('student-profile-detail', kwargs={'pk': self.student_profile.pk})
    ...
    def test_student_retrieve_own_profile(self):
        auth_header = self.get_jwt_header(self.student_user)
        self.client.credentials(HTTP_AUTHORIZATION=auth_header)
        response = self.client.get(self.detail_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['enrollment_number'], 'ST12345')
    ...
    def test_faculty_can_list_profiles(self):
